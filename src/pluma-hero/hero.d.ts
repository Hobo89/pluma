...
export interface HeroLabels { eyebrow: string; about: string; prices: string; vouchers: string; book: string; menu: string; close: string; navigation: string; home: string; pause: string; play: string; photoAlt: string; language: string }
...
export interface HeroController { seek(time: number): void; playFrom(time?: number, rate?: number): void; getState(): {time: number; duration: number; playing: boolean; editing: boolean}; getSequences(): Record<HeroSequenceName, HeroSequence>; setSequences(updates: Partial<Record<HeroSequenceName, Partial<HeroSequence>>>): void; resetSequences(): void; finish(): void; replay(): void; destroy(): void; setLanguage(language: HeroLanguage, notify?: boolean): void }
